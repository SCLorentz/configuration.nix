...
struct Window *create_window(GdkMonitor *monitor)
{
    ...
    if (monitor == NULL) g_warning("\"monitor\" not set, what?");
    ...
    if (!w->monitor) g_warning("no monitor set, wtf?");
