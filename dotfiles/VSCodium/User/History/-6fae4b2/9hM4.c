...
void window_configure(struct Window *w)
{
    ...
    g_print("Backend: %s\n", gdk_display_get_name(gdk_display_get_default()));
        ...
        {
            g_warning("err while settings use_layer_shell");
            exit(1);
        }
        gtk_layer_init_for_window(GTK_WINDOW(w->window));
        gtk_layer_set_exclusive_zone(GTK_WINDOW(w->window), -1);
        gtk_layer_set_layer(GTK_WINDOW(w->window), GTK_LAYER_SHELL_LAYER_TOP);
        gtk_layer_set_monitor(GTK_WINDOW(w->window), w->monitor);
        gtk_layer_set_anchor(GTK_WINDOW(w->window), GTK_LAYER_SHELL_EDGE_LEFT, TRUE);
        gtk_layer_set_anchor(GTK_WINDOW(w->window), GTK_LAYER_SHELL_EDGE_RIGHT, TRUE);
        gtk_layer_set_anchor(GTK_WINDOW(w->window), GTK_LAYER_SHELL_EDGE_TOP, TRUE);
        gtk_layer_set_anchor(GTK_WINDOW(w->window), GTK_LAYER_SHELL_EDGE_BOTTOM, TRUE);
    #endif
        window_setup(w);
