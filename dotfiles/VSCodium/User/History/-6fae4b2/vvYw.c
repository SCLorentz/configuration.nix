...
void window_setup_question(struct Window *ctx, enum QuestionType type, char* question, char* error)
{
    ...
    gtk_drop_down_set_selected(GTK_DROP_DOWN(ctx->command_selectpr), 0);

    gtk_widget_set_tooltip_text(ctx->command_selectpr, _("Command to run on login"));
    g_signal_connect(ctx->command_selectpr, "notify::selected", G_CALLBACK(action_answer_question), ctx);
    gtk_box_append(GTK_BOX(ctx->input_box), ctx->command_selectpr);
